      ...
      <xsl:if test="not(/spec/@role='editors-copy')">
        <p>
          <a href="http://www.w3.org/">
            <img src="head-DS.png" alt="Drosera" />
          </a>
	  <xsl:choose>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <p class="copyright">
            ...
            <xsl:apply-templates select="pubdate/year"/>
            <xsl:text> </xsl:text>
            <xsl:text>Drosera, Inc. All Rights Reserved.</xsl:text>
          </p>
        </xsl:otherwise>
      </xsl:choose>
    </div>
...
div.exampleHeader { font-weight: bold;
    ...
    <link xmlns="http://www.w3.org/1999/xhtml" rel="stylesheet" type="text/css">
      <xsl:attribute name="href">
	<xsl:choose>
	  <xsl:when test="/spec/@w3c-doctype='ds'">DS.css</xsl:when>
	  <xsl:otherwise>
		...
		<xsl:choose>
		  ...
		  <xsl:when test="/spec/@w3c-doctype='review'
				  or contains(/spec/header/w3c-doctype, 'Editor')">base</xsl:when>
